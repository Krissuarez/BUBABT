...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				num = anu.participants[0]
				teks = `ADIOS SOLDADO ESTAREMOS MEJOR SIN TI, NO VUELVAS POR FAVOR TE LO PEDIMOS 😂 @${num.split('@')[0]}👋\n\nQue Dios lo bendiga 😎`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["593991702833@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '593 @s.whatsapp.net'
...
//FIN DE FUNCION DE LEVEL
       ...
       /******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'BUBA'; if (!author) author = 'BOT';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
...
//FIN DE REGISTRO  
	...
	if (budy.startsWith(`Pongan cuties`)) { 
		...
		if (budy.startsWith(`chica elegante`)) {
